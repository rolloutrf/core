  ...
  return (
    <section className="py-24 px-6 overflow-hidden relative">
      ...
      <button
        ...
        className="absolute left-6 top-1/2 -translate-y-1/2 z-10 size-12 rounded-full bg-[#E8552D] hover:bg-[#E8552D]/80 disabled:opacity-0 flex items-center justify-center transition-all"
      >
      ...
      <button
        ...
        className="absolute right-6 top-1/2 -translate-y-1/2 z-10 size-12 rounded-full bg-[#E8552D] hover:bg-[#E8552D]/80 disabled:opacity-0 flex items-center justify-center transition-all"
      >
        <ChevronRight className="size-5 text-white" />
      </button>

      <div
        className="-mx-6 pl-[max(1.5rem,calc((100vw-80rem)/2))]"
      >
        <div
          ref={containerRef}
          className="flex gap-6 transition-transform duration-500 ease-in-out"
          style={{ transform: `translateX(-${currentIndex * (CARD_W + GAP)}px)` }}
        >
          {slides.map((slide) => (
            <div key={slide.name} className="flex-none flex flex-col gap-4" style={{ width: CARD_W }}>
              ...
              <h3 className="font-medium text-2xl">{slide.name}</h3>
              <p className="text-base text-muted-foreground leading-relaxed">{slide.role}</p>
            </div>
          ))}
        </div>
      </div>
